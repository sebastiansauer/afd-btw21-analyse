...
date: "Letztes Update: `r Sys.time()`"
dpi: 300
output: 
...
Am einfachsten: Laden Sie den [Github-Projektordner](https://github.com/sebastiansauer/afd-btw21-analyse) für diesen Workshop herunter. 
Dann haben sie gleich alle nötigen Dateien in der gleichen Struktur,
wie in dieser Rmd-Datei aufgeführt.

Wenn Sie auf den grünen Button ("Code") klicken, 
können Sie den ganzen Projektordner herunterladen.
...
Alternativ kann man die Datei selber herunterladen und im Verzeichnis der Rmd-Datei abspeichern. 
Man kann als CSV-Datei oder als XLSX-Datei abspeichern und dann in R importieren.
Etwas nervig ist, dass das deutsche Excel (im Standard) keine Standard-CSV erstellt.
...
`kerg2_aufbereitet.csv`  meint, Sie haben die ersten paar Zeilen selber gelöscht
und insgesamt dafür gesorgt, dass es eine schöne, maschinenfreundliche Datendatei ist.

Eine CSV-Datei, die mit dem deutschen Excel erstellt wurde, können Sie mit `read_csv2(dateiname)` einlesen.

Achtung: Excel schreibt manchmal in Ihre CSV-Datei *auch ohne* dass Sie auf Speichern klicken!
Fragen Sie mich nicht, warum Excel sich diese Freiheit nimmt.
...
Der Befehl könnte voraussetzen, dass der Ordner `objects` existiert.
Alternativ können Sie auch einfach ins aktuelle Verzeichnis schreiben, das ist einfacher (aber nicht so aufgeräumt):


```{r AfD-BTW21-Analyse-01-9a, eval = FALSE}
...
Ich habe diese Version (der Shapefile, also der Geo-Daten) verwendet: 
"Geometrie der Wahlkreise im [Koordinatensystem](https://de.wikipedia.org/wiki/Kartennetzentwurf) [UTM32](https://de.wikipedia.org/wiki/UTM-Koordinatensystem", [generalisierte](https://enterprise.arcgis.com/de/portal/10.5/use/shapefiles.htm) Shape-Datei.
...
Achtung: Wenn Sie die Daten selber herunterladen, 
passen Sie *Ihren* Pfad entsprechend in der Syntax oben an. 

Bei mir gibt es einen Unterordner "data" in dem wiederum ein Unterordner "btw21_geometrie_wahlkreise_shp" existiert.
Darin liegt eine Shape-Datei, auf die ich zugreife.
...
Jetzt haben wir drei Datensätze "gejoint" ("verheiratet"), 
also entsprechend passender Zeilen zusammengefügt.


## Verheiratete Daten in einer Datei sichern
...
Wir speichern nicht als Tabelle, da die Geo-Daten ein komplexeres Format haben.
Stattdessen speichern als "normal" (binäre) Datei im R eigenen RDS-Format.
Das ist ein effizientes, platzsparendes Format.
...
[Hier](https://github.com/sebastiansauer/afd-btw21-analyse/blob/main/objects/d2.rds) finden Sie die Datei in unserem Repo.
[Hier](https://github.com/sebastiansauer/afd-btw21-analyse/raw/main/objects/d2.rds) ist der Link zum direkten Download.

Sie können auch das ganze Repo herunterladen, wie oben erwähnt.
Oder geschickter, Sie nutzen Github Desktop, dann können Sie jeweils die Updates herunterladen (und ggf. hochladen).




Dieses Vorgehen ist hilfreich, falls Sie oben Probleme hatten, alle Daten zu joinen. 
In dem Fall importieren Sie (mit `read_rds()`) einfach den Datensatz `d2`.
...
Andere [Farbpalette](https://spielmanlab.github.io/introverse/articles/color_fill_scales.html):


```{r AfD-BTW21-Analyse-01-32a}
d2 %>% 
  ggplot() +
  geom_sf(aes(fill = Prozent,
              geometry = geometry)) +
...
### Zwei Cluster identifizieren

Nach Analyse der univariaten Statistiken könnte man die Hypothese haben, dass Sachen, Thüringen oder Sachsen-Anhalt zum ersten "oberen" (oben-links im letzten Streudiagramm) gehören. 
...
d2 %>% 
  ...
  mutate(T_S_SA = 
           case_when(
             state == "Thüringen" |
             state == "Sachsen" | 
             state == "Sachsen-Anhalt" ~ TRUE,
             TRUE ~ FALSE
           ))

d2a %>%   
select(Prozent, for_prop, T_S_SA) %>% 
  ggplot() +
  aes(x = for_prop, 
      y = Prozent, 
      color = T_S_SA) +
...
Das hat ganz gut geklappt; die beiden Cluster wurden leidlich identifiziert. 
Sicherlich ist noch Luft nach oben.
Und, natürlich, bleibt die Frage offen, 
welche Eigenschaften jener Bundesländer dazu führt,
dass sie ein Cluster bilden.

### Farben pro Bundesland


```{r AfD-BTW21-Analyse-01-38c}
d2 %>% 
  filter(Gebietsart == "Wahlkreis") %>% 
  ggplot() +
...
Das sind (zu?) viele Farben.
Die Analyse bringt uns nicht wirklich weiter.
...
Alternativ könnte man das Ergebnisobjekt, wie `m1` mit `save(m1, file = "m1.rda")` in eine Datei speichern (und mit `load("m1.rda")` wieder laden).
...
m1_r2 <- bayes_R2(m1)
m1_r2_md <- median(m1_r2)
m1_r2_md
```

Puh, das Modell ist seehr schlecht. Es erklärt kaum mehr als ... nichts (0%).
...
Dieses Modell (`m1`) ist sehr schlecht.
Wir müssen andere oder zusätzliche Prädiktoren bemühen.

Speichern wir die Ergebnisse in unserem Ergebnisobjekt.
...
ggplot(d4) + 
  ...
  geom_abline(
    intercept = median(intercept) +
      median(Sachsen_Thueringen),
    slope = median(for_prop_z),
    data = sample_n(m9_draws, 100),
    size = 1,
    color = "red"
  ) +
  geom_abline(
    intercept = median(m9_draws$intercept) +
      median(m9_draws$Sachsen_Thueringen),
    slope = median(m9_draws$for_prop_z),
    size = 1,
    color = "red"
  ) +  
